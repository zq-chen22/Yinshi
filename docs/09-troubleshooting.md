...
新群会在创建时直接写入描述；已有群需要应用身份权限 `im:chat:update`。在飞书开放平台添加“更新群信息”权限、创建并发布新应用版本，然后重启桥或等待下一次退避重试。若日志出现错误码 `99991672`，说明权限尚未开通或最新权限仍未随应用版本发布。
...
- 正常 `systemctl restart`：新版本会先排空活动 turn，通常不应再产生基础设施 `interrupted`；
- 已取得 turn ID、桥异常重启且旧 turn 无最终答复：新进程应显示“Codex 正在恢复”，在同一 thread 检查现场并续做；
- `turn/start` 返回前就中断、提交边界不确定：用 `待确认`；
- thread 被安全锁定：本机核对后 `解除线程`；
- 明确需要从头执行：使用 `重试 消息ID`，并接受副作用可能重复。

如果恢复后又看到“来自主机侧”的内容重复了原始飞书指令，说明部署仍在运行旧版本，或旧 turn 没有被识别为桥自有 turn。检查服务实际加载的源码路径、版本、`turn_jobs` 关联和最近启动日志。新版本会先把桥自有旧 turn 标记为已同步，外部更新扫描器也会排除所有 `turn_jobs` 中的 turn。
...
- 短暂断网：长连接自动重建，持久队列继续；
- 机器完全关机：期间无法实时处理；
- 正常服务重启：先排空已接单任务，新消息留在持久 inbox 等下一进程；
- 桥自有 turn 已有 ID、明确被中断且无最终答复：同 thread 检查现场后续做；
- 处于 `turn/start` 不确定临界区：不会自动重放，进入 `待确认`。
